...
function LancRow({ l, idx, bulkMode, selected, toggleSelect, onEdit, onDelete, onView }) {
  ...
  return (
    ...
    >
      ...
      <td style={{ textAlign: 'center' }}>{l.cat ? <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', color: '#d97757', background: '#fff4ef', border: '1px solid #d97757', borderRadius: 4, padding: '2px 7px', whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: 1.4, display: 'inline-block' }}>{l.cat}</span> : <span style={{ color: '#aaa' }}>—</span>}</td>
      ...
      <td style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: '#333333', textAlign: 'center', whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: 1.3 }}>{l.ccpgto || '—'}</td>
      <td>
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, zIndex: 999, background: '#fff', border: '1px solid #e8e8e5', borderRadius: 10, boxShadow: '0 4px 16px rgba(0,0,0,0.10)', minWidth: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ overflowY: 'auto', maxHeight: 220 }}>
            <div
              ...
              style={{ padding: '8px 14px', fontSize: 13, color: '#999', cursor: 'pointer', background: !value ? '#fff4ef' : 'transparent' }}
              onMouseEnter={e => e.currentTarget.style.background = '#f5f5f3'}
              onMouseLeave={e => e.currentTarget.style.background = !value ? '#fff4ef' : 'transparent'}
            >{placeholder}</div>
            {filtered.map(o => (
              <div key={o} onClick={() => { onChange(o); setOpen(false); }}
                style={{ padding: '8px 14px', fontSize: 13, color: '#333', cursor: 'pointer', background: value === o ? '#fff4ef' : 'transparent' }}
                onMouseEnter={e => e.currentTarget.style.background = '#f5f5f3'}
                onMouseLeave={e => e.currentTarget.style.background = value === o ? '#fff4ef' : 'transparent'}
              >{o}</div>
            ))}
            {filtered.length === 0 && <div style={{ padding: '8px 14px', fontSize: 12, color: '#aaa' }}>Nenhum resultado</div>}
          </div>
        </div>
      )}
    </div>
